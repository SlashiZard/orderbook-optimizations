 ...
 */
void VanillaOrderbook::CancelOrders(OrderIds orderIds) {
 ...
 */
void VanillaOrderbook::CancelOrderInternal(OrderId orderId) {
	orders_.erase(
 ...
 * Runs in O(N) where N is the amount of orders.
 */
const OrderPointer VanillaOrderbook::getBestAsk() const {
 ...
 * Runs in O(N) where N is the amount of orders.
 */
const OrderPointer VanillaOrderbook::getBestBid() const {
 ...
 * Runs in O(N) where N is the amount of orders.
 */
const OrderPointer VanillaOrderbook::getWorstAsk() const {
	OrderPointer bestAskOrder = nullptr;
 ...
 * Runs in O(N) where N is the amount of orders.
 */
const OrderPointer VanillaOrderbook::getWorstBid() const {
	OrderPointer bestBidOrder = nullptr;
...
bool VanillaOrderbook::orderExists(OrderId orderId) const {
	return std::any_of(orders_.begin(), orders_.end(), [&](const OrderPointer& o) {
		return o && o->GetOrderId() == orderId;
	});
}

bool VanillaOrderbook::CanMatch(Side side, Price price) const {
	if (orders_.empty())
		return false;
 ...
 */
bool VanillaOrderbook::CanFullyFill(Side side, Price price, Quantity quantity) const {
	for (const auto& order : orders_) {
 ...
 */
Trades VanillaOrderbook::MatchOrders() {
	Trades trades;
 ...
 * Runs in O(N * log(M)) where N is the total amount of orders and M is the amount of price levels.
 */
Trades VanillaOrderbook::AddOrder(OrderPointer order) {
 ...
 */
void VanillaOrderbook::CancelOrder(OrderId orderId) {
	CancelOrderInternal(orderId);
}
 ...
 * Runs in O(N * log(M)) where N is the total amount of orders and M is the amount of price levels.
 */
Trades VanillaOrderbook::ModifyOrder(OrderModify order) {
	OrderType orderType;
 ...
 * Runs in O(1).
 */
std::size_t VanillaOrderbook::Size() const {
	return orders_.size();
}

/* Generates a snapshot of the aggregated orderbook based on the selected strategy.
 */
OrderbookLevelInfos VanillaOrderbook::GetOrderInfos() const {
